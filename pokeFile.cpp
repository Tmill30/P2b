...
vector<Pokemon> readFiles() {
    ...
    while (getline(pokemonFile, line)) {
        ...
        try {
            ...
            std::cerr << "Invalid dex number: " << temp << std::endl;
            continue;
        }

        std::cout << "Dex #: " << pokemon.num << std::endl;
        ...
        std::cout << "Name: " << pokemon.name << ", Gen: " << pokemon.generation << std::endl;

        if (!pokemon.name.empty() && pokemon.num > 0 && pokemon.generation >= 1 && pokemon.generation <= 9) {
            Pokedex.push_back(pokemon);
        } else {
            std::cerr << "Skipped invalid entry. Name: '" << pokemon.name << "', Gen: " << pokemon.generation << ", Num: " << pokemon.num << std::endl;
        }
    }
